...
class FreeRex(Optimizer):
    ...
    Arguments:
        params (iterable): iterable of parameters to optimize or dicts defining
            parameter groups
        k_inv (float, optional): learning rate analogue (specifies a point on optimal regret frontier) (default: 1/sqrt(5))
        weight_decay (float, optional): weight decay (L2 penalty) (default: 0)
    """

    def __init__(self, params, k_inv=1.0/np.sqrt(5.0), weight_decay=0):
        defaults = dict(k=1.0/k_inv, weight_decay=weight_decay)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if p.grad.data.is_sparse:
                    ...
                    p.data[grad_indices] = -torch.sign(sparse_grad_sum) * (torch.exp(torch.abs(sparse_grad_sum)/(group['k'] * torch.sqrt(sparse_one_over_eta_sq))) - 1.0)
                    ...
                    p.data = -torch.sign(state['grad_sum']) * (torch.exp(torch.abs(state['grad_sum'])/(group['k']*torch.sqrt(state['one_over_eta_sq']))) - 1.0)/np.sqrt(state['max_l1'])
